...
path_images_from_camera = r"visitor_data\\"


class Face_Register:
    def __init__(self):
        self.path_photos_from_camera = r"visitor_data\\"
...
def main():
    ...
    person_list = os.listdir(r"visitor_data\\")
    person_list.sort()
    with open(r"visitor_data\visitor_features.csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        for person in person_list:
            person_path = os.path.join(r"visitor_data\\", person)
            if os.path.isdir(person_path):
                ...
                logging.info('\n')
        logging.info("所有录入人脸数据存入 / Save all the features of faces registered into: data/visitor_features.csv")
    familiy_features = pd.read_csv(r'family_member_data\family_features.csv',header=None) 
    visitor_features = pd.read_csv(r'visitor_data\visitor_features.csv',header=None)
    combined_features = pd.concat([familiy_features, visitor_features])
